...
```{r ans12}
# Game5Time
Game5Time <- list("Date"=Game5$Date,
                  "StartTime"=Game5$StartTime,
                  "TimeZone"=Game5$TimeZone)
...
G5TimeParsed <- c(Game5Time[[1]],Game5Time[[2]],Game5Time[[3]])
...
```{r ans15}
lubridate::minute(G5TimeParsed) <- 15
ParisLouis <- with_tz(G5TimeParsed, tzone = "Europe/Paris")
```
...
```{r ans21}
chNumbers_raw <- factor(chNumbers, levels = c("壹","貳","參","肆","伍","陸","柒","捌","玖"))       
chNumbers <- as.integer(chNumbers_raw)
...
```{r ans22}
chNumbersWith0_raw <- factor(chNumbersWith0, levels =
                      c("零","壹","貳","參","肆","伍","陸","柒","捌","玖"))
chNumbersWith0 <- as.integer(chNumbersWith0_raw)
sumChNumbersWith0 <- sum(chNumbersWith0) - length(chNumbersWith0)
```
...
```{r ans31}
scapeText <- paste0("在字串輸入時我們要用成對的\"或\'將文字包起來，R才會知道它是字串。",
                    "\n字串在輸入時，若要cat時在螢幕看起來有換行則必需輸入\\n，其中\\稱為Escape key，它讓後面字元跳脫了它在電腦的原始含意。")
cat(scapeText)
```
...
```{r ans32}
dearStudent <- c(paste0("Dear ", student$name,
                        ": \n\n 貼心提醒您，作業上傳截止時間是", substr(student$deadline,1 , 10), "T", substr(student$deadline, 12, 19), "+0000",   
                        "\n\n 敬祝學安", 
                        "\n\n 你的貼心學伴巴西利bot silly"))
cat(dearStudent)
```
